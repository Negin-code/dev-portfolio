    ...
    >
      ...
      <div className="col-start-1 row-start-1 self-center pt-10 md:pt-5">
        {lines.map(({ text, outline, delay }, i) => (
          <span
            key={text}
            className={`block overflow-hidden text-[clamp(2.2rem,11vw,7rem)] md:text-[clamp(3rem,12vw,8rem)] xl:text-[clamp(4rem,13vw,12rem)] leading-[0.88] tracking-[-0.04em] ${outline ? "text-outline-dark" : "text-ink"}`}
